...
def read_label(label_filename):
    lines = [line.rstrip() for line in open(label_filename)] #douga 删除了最后一位数字
...
def init_bev_image(out_size=1080):
    bird_view = np.ones((out_size, out_size, 3), dtype=np.uint8) * 230

    return bird_view

def draw_bev_box3d(image, boxes3d, color=(255, 255, 255), cls=None, thickness=1, scores=None, text_lables=[], world_size=40, out_size=1080):
    if color is None:   
        if cls is not None:
            if cls == 'Car':
                # color = (55, 67, 225)
    ...
    for idx in range(boxes3d.shape[0]):
        ...
        # cv2.line(image,(pts[0][0], pts[0][1]), (pts[1][0],pts[1][1]), (0,0,255), thickness, lineType=cv2.LINE_AA)
        cv2.line(image,(pts[0][0], pts[0][1]), (pts[3][0],pts[3][1]), (0,0,255), thickness, lineType=cv2.LINE_AA)
        # print(rf'pts: {pts}')
    return image
...
def resizing(pts):
    ...
    # 定义放大比例
    scale_factor = 1.5  # 你可以根据需要调整放大比例
    ...
    # 将边界框的左上角坐标和宽度、高度转换回点的坐标
    enlarged_pts = np.array([
        [new_x_min, new_y_min],
        [new_x_min + new_width, new_y_min],
        [new_x_min + new_width, new_y_min + new_height],
        [new_x_min, new_y_min + new_height]
    ], dtype=np.int)
    return enlarged_pts
...
def show_image_with_boxes(image, cls_ids, target_center, box2d, corners_2d, reg_mask, offset_3D, down_ratio, pad_size, encoded_alphas, vis=True, index=[], img_idx=0):
    ...
    cv2.imwrite("/home/fansq/playground/Mono/output/debug/{}.png".format(img_idx), cv2.cvtColor(stacked_img,cv2.COLOR_RGB2BGR))
...
def show_heatmap(img, heat_map, classes=['Car'], index=None):
    colors = [(0, 1, 0), (1, 0, 0), (0, 0, 1)]
    ignored_color = np.array([1, 1, 1])

    resized_img = cv2.resize(img,(heat_map.shape[2], heat_map.shape[1]))
    ...
    cv2.imwrite("./debug/{}_cvmap.png".format(index), mix_img*255)

def show_bevmap(heat_map, classes=['Car'], index=None):
    colors = [(0, 1, 0), (1, 0, 0), (0, 0, 1)]
    ignored_color = np.array([1, 1, 1])

    mix_img = np.zeros((heat_map.shape[2], heat_map.shape[1], 3)) / 255
    all_heat_img = np.zeros_like(mix_img)
    ...
    cv2.imwrite("./output/vis/{}_bevpred.png".format(index), all_heat_img*255)
