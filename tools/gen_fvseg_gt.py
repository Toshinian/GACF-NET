...
def get_args():
    ...
    parser.add_argument("--range_x", type=int, default=175, help="Size of the rectangular grid in metric space (in m)")
    parser.add_argument("--range_y", type=int, default=15, help="Size of the rectangular grid in metric space (in m)")
    ...
    for label in labels:
        ...
        if elems[0] in ['Garbage','Ship','SmallBoat']:
            if elems[0] == 'Garbage':
                ...
                center_x = int(float(elems[11]) / res_x2 + width2 / (2 * res_y2))
                # center_y = int(float(elems[12]))
                center_y = int(float(elems[12]))
                # print (center_y, int(float(elems[8])), float(elems[12]), float(elems[8]))
                orient = -1 * float(elems[14])

                obj_h = int(float(elems[8]) / res_y2)
                obj_l = int(float(elems[10]) / res_x2)
                obj_w = int(float(elems[9]) / res_x2)

                rectangle_bev = get_rect(0, 0, obj_l, obj_w, orient)
                ori_l = max(rectangle_bev[:, 1]) - min(rectangle_bev[:, 1])


                rectangle = get_rect(center_x, int(width2 / res_y2) -int(obj_h/2), ori_l, obj_h, 0)
...
if __name__ == "__main__":
    args = get_args()
    args.out_dir = '/datasets/FlowSense_BEV/training/fv_seg'.format(args.dataset)
